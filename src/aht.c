...
// Initialize the aht 'aht' to hold 'sz' leaves
...
static void aht_swap(struct aht* aht, struct aht_node* a, struct aht_node* b){ // require 'a' to be subordinate to 'b' (which implies a->block_next >= 0 and b->block_prev >= 0)
	/*
	swap 'a' and 'b'
	different routines based on adjacency (adj) of a and b:
		adj = true:
			list before: ... <-> x <-> a <-> b <-> z <-> ...
			list after:  ... <-> x <-> b <-> a <-> z <-> ...
		adj = false:
	...
	*/
	...
	if (adj)
		...
		b->block_next = t; // --------------------|non-adj|-------- ...     x  -> b  -> [...y...]     a  -> z     ...
		aht->tree[b->block_prev].block_next = a - aht->tree; // -n- ...     x  -> b  -> [...y...]  -> a  -> z     ...
	}
	// backward arrows
	...
	if (adj)
		...
		a->block_prev = b->block_prev; // --------|non-adj|-------- ...     x  -> b  -> [...y...] <-> a <-> z     ...
	b->block_prev = t; // ----------------------------------------- ...     x <-> b  -> [...y...] <-> a <-> z     ...
	if (!adj)
		aht->tree[b->block_next].block_prev = b - aht->tree; // -n- ...     x <-> b <-> [...y...] <-> a <-> z     ...
